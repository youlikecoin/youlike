...
const char * const BITCOIN_CONF_FILENAME = "youlike.conf";
const char * const BITCOIN_PID_FILENAME = "youliked.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "youlike" is a composite category enabling all YouLike-related debug output
                if(ptrCategory->count(std::string("youlike"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\YouLike
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\YouLike
    // Mac: ~/Library/Application Support/YouLike
    // Unix: ~/.youlike
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "YouLike";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/YouLike";
#else
    // Unix
    return pathRet / ".youlike";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty youlike.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override youlike.conf
