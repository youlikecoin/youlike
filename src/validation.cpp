...
#if defined(NDEBUG)
# error "YouLike Core cannot be compiled without assertions."
#endif
                              ...
                              const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    BOOST_FOREACH(const CTxIn &txin, tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in YouLike
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    // Hard fork to reduce the block reward by 4 extra percent (allowing budget/superblocks)
    CAmount nSuperblockPart = (nPrevHeight > consensusParams.nBudgetPaymentsStartBlock) ? nSubsidy * 0.04 : 0;
...
static int64_t nTimeYouLikeSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    // YLI : CHECK TRANSACTIONS FOR INSTANTSEND
        ...
        for (const auto& tx : block.vtx) {
            ...
            for (const auto& txin : tx->vin) {
                ...
                if (instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx->GetHash()) {
                    ...
                    return state.DoS(10, error("ConnectBlock(YLI): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), hashLocked.ToString()),
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
                return state.DoS(10, error("ConnectBlock(YLI): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else {
        LogPrintf("ConnectBlock(YLI): spork is off, skipping transaction locking checks\n");
    }
    ...
    // YLI : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(YLI): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(YLI): couldn't find masternode or superblock payments"),
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(YLI): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeYouLikeSpecific += nTime5 - nTime4;
    LogPrint("bench", "    - YouLike specific: %.2fms [%.2fs]\n", 0.001 * (nTime5 - nTime4), nTimeYouLikeSpecific * 0.000001);
