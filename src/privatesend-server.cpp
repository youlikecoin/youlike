...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all YouLike related functionality
...
/*
    ...
    allow endless transaction that would bloat YouLike and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all YouLike specific functionality
