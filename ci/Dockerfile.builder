...
# youlike_hash
RUN git clone https://github.com/youlikepay/youlike_hash
RUN cd youlike_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} youlike
RUN useradd -u ${USER_ID} -g youlike -s /bin/bash -m -d /youlike youlike
...
RUN mkdir /youlike-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /youlike-src && \
