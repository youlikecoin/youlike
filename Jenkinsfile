...
// which are part of the YouLike repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="youlike-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /youlike-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "youlikepay-youlike/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from youlikepay-youlike/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'youlikepay-youlike/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from youlikepay-youlike/develop"
            }
          }

          if (hasCache) {
            sh "cd /youlike-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /youlike-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /youlike-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /youlike-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /youlike-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /youlike-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /youlike-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
